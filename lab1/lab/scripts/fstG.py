...
train_gr = "../data/train_gr.txt"
train_greng = "../data/train_greng.txt"
...
def main():
	greek = readContent(train_gr)
	greng = readContent(train_greng)
...
def matchingRules(greek, greng, file='convertions.txt', out=True):
	...
	for gr_word, en_word in content:
		...
		while gr_index < gr_length and en_index < en_length:
			...
			if last_greek and gr_word[gr_index] + gr_word[gr_index + 1] in sp_ch_gr.keys() and en_word[en_index] in sp_ch_gr[gr_word[gr_index] + gr_word[gr_index + 1]]:
				if (last_greng and gr_word[gr_index] == u'Λ' and gr_word[gr_index + 1] == u'Λ' and en_word[en_index] == 'L' and en_word[en_index + 1] == 'L') \
					or (last_greng and gr_word[gr_index] == u'Β' and gr_word[gr_index + 1] == u'Β' and en_word[en_index] == 'V' and en_word[en_index + 1] == 'V') \
					or (last_greng and gr_word[gr_index] == u'Γ' and gr_word[gr_index + 1] == u'Κ' and en_word[en_index] == 'G' and en_word[en_index + 1] == 'K'):
					en_key = en_word[en_index]; gr_key = gr_word[gr_index]
				else:
				...
				if en_word[en_index] == 'T' and en_word[en_index + 1] == 'H' and gr_word[gr_index] == u'Τ':
					en_key = en_word[en_index]; gr_key = gr_word[gr_index]
				else: en_key = en_word[en_index] + en_word[en_index + 1]; gr_key = gr_word[gr_index]; en_index += 1
...
def createFst(rules, n, gr_syms, en_syms, file='generic.fst'):
	...
	for keys, values in rules.items():
		for key, value in values.items():
			...
			if len(keys) == len(key):
				fd.write('0\t1\t' + keys + '\t' + key + '\t' + str(weight) + '\n')
			elif len(keys) == 2:
				state += 1
				fd.write('0\t' + str(state) + '\t' + keys[0] + '\teps\t' + str(weight) + '\n')
				fd.write(str(state) + '\t1\t' + keys[1] + '\t' + key + '\t1\n')
			elif len(key) == 2:
				state += 1
				fd.write('0\t' + str(state) + '\teps\t' + key[0] + '\t' + str(weight) + '\n')
				fd.write(str(state) + '\t1\t' + keys + '\t' + key[1] + '\t1\n')
	fd.write("1\n")
	fd.close()
